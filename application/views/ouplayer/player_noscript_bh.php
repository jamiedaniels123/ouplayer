...
if ($standalone):
?>
<!DOCTYPE html><html lang="en"><meta charset="utf-8" /><title><?=$meta->title ?> | <?=t('OU player (BH)') ?></title>
...
<?php if ($meta->media_html5): ?>
  <?php if ($meta->media_type=='video'): ?>
    <video src="<?=$meta->media_url ?>" autobuffer controls poster="<?=$meta->poster_url ?>" width="<?=$meta->width ?>" height="<?=$meta->height ?>"></video>
  <?php else: ?>
    <audio src="<?=$meta->media_url ?>" autobuffer controls width="<?=$meta->width ?>" height="<?=$meta->height ?>"></audio>
  <?php endif; ?>
<?php endif; ?>
</object>
